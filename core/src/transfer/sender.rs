...
/// Like [`build_transfer`] but encrypts the compressed data with `session_key`
/// before sharding.  Uses [`crypto::encrypt`] which embeds a random nonce.
///
/// The receiver must use the same `session_key` to decrypt via [`crypto::decrypt`].
pub fn build_transfer_encrypted(
    name: &str,
    content_type: &str,
    data: &[u8],
    allow_compression: bool,
    session_key: &[u8; crypto::KEY_LEN],
) -> Result<TransferPlan, TransferError> {
    let name = safe_file_name(name);
    let content_type = truncate_utf8(content_type, 52);
    if MANIFEST_FIXED + name.len() + content_type.len() > MAX_PAYLOAD {
    ...
    let (to_encrypt, compression) = match compressed {
        Some(value) if value.len() + 32 < data.len() => (value, Compression::Zstd),
        _ => (data.to_vec(), Compression::None),
    };

    let encrypted =
        crypto::encrypt(&to_encrypt, session_key).map_err(|_| TransferError::Malformed("encryption failed"))?;
    ...
    let mut metadata = TransferMetadata {
        ...
        content_type,
        original_size: data.len() as u64,
        encoded_size: encrypted.len() as u64,
        sha256,
        compression,
        group_count: 0,
    };

    if INLINE_FIXED + metadata.name.len() + metadata.content_type.len() + encrypted.len()
        <= MAX_PAYLOAD
    {
        let packet = encode_inline(&metadata, &encrypted)?;
        return Ok(TransferPlan {
            metadata,
            packets: vec![packet],
        });
    ...
    let (data_shards, parity_shards) = shard_geometry(encrypted.len() as u64);
    let group_bytes = data_shards * SHARD_SIZE;
    let groups = encrypted.len().div_ceil(group_bytes);
    metadata.group_count = u32::try_from(groups).map_err(|_| TransferError::TooLarge)?;
    ...
    for group in 0..groups {
        ...
        for (index, shard) in shards.iter_mut().take(data_shards).enumerate() {
            ...
            let end = (start + SHARD_SIZE).min(encrypted.len());
